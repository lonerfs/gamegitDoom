...
int showMainMenu(SDL_Renderer* renderer, TTF_Font* font, int windowWidth, int windowHeight) {
    ...
    MenuButton newGameBtn = {{(float)(windowWidth/2-100), (float)(windowHeight/2-40), 200, 50}, "New Game", false};
    MenuButton exitBtn = {{(float)(windowWidth/2-100), (float)(windowHeight/2+20), 200, 50}, "Exit", false};
    ...
    while (menuRunning) {
        ...
        for (auto* btn : {&newGameBtn, &exitBtn}) {
    ...
    while (currentLevel <= 3) {
        ...
        while (!levelComplete) {
            ...
            for (const auto& mobRef : sortedMobs) {
                ...
                float floorY = WINDOW_HEIGHT/2 + (VIEW_HEIGHT) / dist * PROJ_SCALE;
                ...
                if (mob.type == BOSS) {
                    if (isDying && bossDeathTex.texture) {
                        mobTex = &bossDeathTex;
                    } else if (isAttackingNow && bossAttackTex.texture) {
                    ...
                    if (isDying && mobDeathTex.texture) {
                        mobTex = &mobDeathTex;
                    } else if (isAttackingNow && mobAttackTex.texture) {
                        mobTex = &mobAttackTex;
